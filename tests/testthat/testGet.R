...
test_that("Non-transitive dependency download generates correct cmd option", {
  cmd <- .download_dependency_cmd('bar','com.foo','1.0-SNAPSHOT',mvn='/usr/bin/mvn', transitive=FALSE)
...
test_that("Dependency with no transitive arg is transitive by default", {
  cmd <- .download_dependency_cmd('bar','com.foo','1.0-SNAPSHOT',mvn='/usr/bin/mvn')
